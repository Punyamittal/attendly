...
import { createQrSession } from '@/services/attendance'
...
export function DynamicQrGenerator({ registrationNumber }: Props) {
  ...
  const generate = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const session = await createQrSession(registrationNumber)
      ...
      toast.error(message)
      // Back off so a failed generate does not spam every 250ms
  ...
  function downloadPng() {
    ...
    a.href = dataUrl
    a.download = `qr-${registrationNumber}.png`
    a.click()
  ...
  return (
    <>
      <GlassCard className="flex w-full flex-col items-center overflow-hidden text-center">
        <h2 className="font-display text-lg sm:text-xl">Your Attendance QR</h2>
        <p className="mt-1 max-w-sm font-mono text-[10px] text-[var(--muted)] sm:text-xs">
          Refreshes every 60 seconds to prevent screenshot sharing
        </p>
